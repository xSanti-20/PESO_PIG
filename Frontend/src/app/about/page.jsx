...
import { useMobile } from "@/hooks/use-mobile" // Importar el hook

export default function QuienesSomos() {
  const { isMobile, isTablet, isDesktop } = useMobile() // Usar el hook
  ...
  // Función helper para clases responsive
    ...
    <>
      ...
      <div className={`container mx-auto px-4 ${getResponsiveClasses("py-4", "py-6", "py-8")}`}>
        ...
        <div className={`flex justify-center items-center gap-4 mb-8 ${getResponsiveClasses("px-2", "px-4", "px-6")}`}>
          {/* Botón anterior - Ocultar en móvil muy pequeño */}
          <button
            onClick={handlePrev}
          ...
          <Card
            ...
            ${getResponsiveClasses(
              "w-full max-w-sm", // Móvil: card vertical compacta
              "md:flex-row max-w-2xl", // Tablet: card horizontal mediana
              "md:flex-row max-w-4xl", // Desktop: card horizontal grande
            )}
          `}
          >
            {/* Imagen */}
            <div
              className={`
              ${getResponsiveClasses(
                "w-full h-48", // Móvil: imagen arriba, altura fija
                "w-full md:w-1/2 h-48 md:h-64", // Tablet: mitad del ancho
                "w-full md:w-1/2 h-64 md:h-80", // Desktop: mitad del ancho, más alta
              )}
            `}
            >
              <img
            ...
            <CardContent
              ...
              ${getResponsiveClasses(
                "md:text-center", // Móvil: centrado
                "md:w-1/2 md:p-5 md:text-left", // Tablet: izquierda
                "md:w-1/2 md:p-6 md:text-left", // Desktop: izquierda
              )}
            `}
            >
              ...
              <p
                className={`
                mb-4 text-justify
                ${getResponsiveClasses("text-xs leading-relaxed", "text-sm", "text-sm")}
              `}
              >
                {member.description}
              </p>
              ...
              <div
                className={`
                flex items-center text-green-600 gap-2
                ${getResponsiveClasses("justify-center", "justify-center md:justify-start", "justify-center md:justify-start")}
              `}
              >
